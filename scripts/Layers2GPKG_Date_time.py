# coding: utf-8
"""
Layer → GeoPackage (Export, immer neu & Datum)
...
class Layers2GPKG(QgsProcessingAlgorithm):
    ...
    def name(self): return "layers_to_gpkg_new_dt"
    def displayName(self): return "Layer → GeoPackage (neu & Datum)"
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # >>> Zeitstempel anfügen (z. B. __2025-09-15_1405)
        base, ext = os.path.splitext(gpkg_path)
        ext = ext or ".gpkg"
        ts  = dt.datetime.now().strftime("%Y-%m-%d_%H%M")
        gpkg_path = f"{base}__{ts}{ext}"
